...
class XLDatasetBuilder:
    ...
    def enrich_is_home_from_game_logs(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if self.verbose:
                print(
                    f"   Querying game logs for {len(unique_props):,} unique player-date combinations..."
                )
            ...
            if self.verbose:
                print(
                    f"   ✅ Matched is_home for {matched_is_home:,}/{len(df):,} props ({match_rate_is_home:.1f}%)"
                )
                print(
                    f"   ✅ Matched opponent_team for {matched_opponent:,}/{len(df):,} props ({match_rate_opponent:.1f}%)"
                )
            ...
            if still_null_opponent > 0:
                ...
                print(
                    f"   ⚠️  WARNING: {still_null_opponent:,} props missing opponent_team ({missing_pct:.1f}%)"
                )
            ...
            if still_null_is_home > 0:
                ...
                if fallback_pct > 20.0:
                    print(
                        f"   ❌ VALIDATION FAILED: {fallback_pct:.2f}% props missing is_home (threshold: 20%)"
                    )
            ...
            if self.verbose:
                print(
                    f"   📊 is_home distribution: {home_count:,} home ({home_pct:.1f}%), {away_count:,} away ({100-home_pct:.1f}%)"
                )
                print(
                    f"   📊 opponent_team coverage: {valid_opponents:,}/{len(df):,} ({opponent_pct:.1f}%)"
                )

            # Assert realistic distribution (45-55% home games)
            if not (45 <= home_pct <= 55):
                print(
                    f"   ⚠️  WARNING: Suspicious home percentage: {home_pct:.1f}% (expected 45-55%)"
                )

            # Assert good opponent coverage (should be >90% after enrichment)
            if opponent_pct < 90:
                print(
                    f"   ⚠️  WARNING: Low opponent_team coverage: {opponent_pct:.1f}% (expected >90%)"
                )
    ...
    def aggregate_book_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if null_opponents > len(grouped) * 0.5:
            raise ValueError(
                f"CRITICAL: {null_opponents}/{len(grouped)} props have NULL opponent_team!"
            )

        if self.verbose:
            valid_opponents = len(grouped) - null_opponents
            print(
                f"   ✅ opponent_team validation: {valid_opponents:,}/{len(grouped):,} props have valid opponent ({100*valid_opponents/len(grouped):.1f}%)"
            )
        ...
        if self.verbose:
            print(
                f"   Book coverage: {single_book_props} single-book, {multi_book_props} multi-book props"
            )
            print(
                f"   Line spread ≥1.5: {high_spread_props} props ({100*high_spread_props/len(grouped):.1f}%)"
            )
    ...
    def extract_features(self, prop: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not opponent_team or opponent_team == "":
                if self.verbose:
                    print(
                        f"  ⚠️  SKIP: {prop['player_name']} on {prop['game_date']} - missing opponent_team"
                    )
                return None
                ...
                features.update(
                    {
                        ...
                        "line_spread": prop.get("max_line", 0)
                        - prop.get("min_line", 0),  # Compute from min/max
    ...
    def build_dataset(self, stat_type: str) -> pd.DataFrame:
        ...
        if self.verbose and before_dedup != after_dedup:
            print(
                f"   ✅ Deduplicated: {before_dedup:,} → {after_dedup:,} props (removed {before_dedup - after_dedup:,} duplicates)"
            )
        ...
        for _idx, prop in tqdm(
            ...
            try:
                ...
                actual_float = (
                    float(prop["actual_value"]) if prop["actual_value"] not in (None, "") else 0.0
                )
            except (ValueError, TypeError):
                if self.verbose:
                    print(
                        f"  ⚠️  Skipping {prop['player_name']} on {prop['game_date']}: invalid line or actual value"
                    )
                errors += 1
                continue
        ...
        if self.verbose:
            ...
            book_feature_cols = [
                ...
                if "line_spread" in col or "consensus" in col or "num_books" in col
            ]
            if book_feature_cols:
                ...
                for col in book_feature_cols[:5]:
                    ...
                    print(
                        f"   {col}: {non_zero}/{len(dataset)} non-zero ({non_zero_pct:.1f}%), mean={mean_val:.2f}, max={max_val:.2f}"
                    )
            else:
    ...
    def assert_no_leakage(self, df: pd.DataFrame, stat_type: str):
        ...
        print(
            f"   ⏭️  SKIPPED - Feature extractor verified in investigation (all queries use game_date < %s)"
        )
        ...
        feature_cols = [
            c for c in df.columns if c.startswith(("ema_", "h2h_", "team_", "opponent_"))
        ]
        actual_col = f"actual_{stat_type.lower()}"
        ...
        assert len(suspicious) == 0, f"❌ Suspicious correlations detected: {suspicious}"
        ...
        if len(val) > 0 and not pd.isna(val_min):
            ...
            print(
                f"      Train: {train['game_date'].min()} to {train['game_date'].max()} ({len(train):,} props)"
            )
            print(
                f"      Val:   {val['game_date'].min()} to {val['game_date'].max()} ({len(val):,} props)"
            )
        else:
    ...
    def generate_validation_report(self, datasets: Dict[str, pd.DataFrame], output_path: Path):
        ...
        for stat_type, df in datasets.items():
            ...
            # Feature statistics
            feature_cols = [
                c for c in df.columns if c.startswith(("ema_", "team_", "opponent_", "h2h_"))
            ]
            ...
            report.append(f"\n**Label Distribution:**")
            report.append(
                f"- OVER (1): {(df['label'] == 1).sum():,} ({(df['label'] == 1).mean()*100:.1f}%)"
            )
            report.append(
                f"- UNDER (0): {(df['label'] == 0).sum():,} ({(df['label'] == 0).mean()*100:.1f}%)"
            )
            ...
            if "line_spread" in df.columns:
                report.append(f"\n**Line Shopping Opportunities:**")
                report.append(
                    f"- Spread ≥1.5: {(df['line_spread'] >= 1.5).sum():,} props ({(df['line_spread'] >= 1.5).mean()*100:.1f}%)"
                )
                report.append(
                    f"- Spread ≥2.5: {(df['line_spread'] >= 2.5).sum():,} props ({(df['line_spread'] >= 2.5).mean()*100:.1f}%)"
                )
        ...
        report.append(
            f"**Features per Prop:** 130 (Phase 3: 75 player + 23 book + 22 H2H + 2 position + 4 interaction + 4 computed)"
        )
        ...
        try:
            ...
            for stat_type in STAT_TYPES:
                ...
                if dataset is not None and len(dataset) > 0:
                    ...
                    # Save to CSV
                    output_file = (
                        self.output_dir / f"xl_v2_matchup_training_{stat_type}_2023_2025.csv"
                    )
...
def main():
    ...
    parser.add_argument(
        "--output", type=str, default="datasets/", help="Output directory for CSV files"
    )
    parser.add_argument(
        "--market",
        type=str,
