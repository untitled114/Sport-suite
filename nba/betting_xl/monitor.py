...
class PerformanceMonitor:
    ...
    def calculate_rolling_performance(self, days: int = 7) -> Dict[str, Dict]:
        ...
        recent_picks = [
            p for p in self.picks_data.get('picks', [])
        ...
        if overall_metrics['total_picks'] > 0:
            ...
            if completed > 0:
                ...
                overall_metrics['roi'] = ((overall_metrics['wins'] * 0.91 - overall_metrics['losses']) / completed) * 100
    ...
    def check_stop_loss_triggers(self, performance: Dict) -> Tuple[str, List[str]]:
        ...
        for market, metrics in performance['by_market'].items():
            ...
            if benchmark_wr > 0:
                ...
                if wr_drop > 15:
                    reasons.append(f"{market} WR dropped {wr_drop:.1f}pp vs validation ({benchmark_wr:.1f}%)")
                    market_statuses.append('STOP')
                elif wr_drop > 10:
                    reasons.append(f"{market} WR dropped {wr_drop:.1f}pp vs validation ({benchmark_wr:.1f}%)")
        ...
        if consecutive_losses >= MAX_CONSECUTIVE_LOSING_DAYS:
            reasons.append(f"{consecutive_losses} consecutive losing days (max: {MAX_CONSECUTIVE_LOSING_DAYS})")
            market_statuses.append('STOP')
    ...
    def check_edge_anomalies(self, today_picks: List[Dict]) -> Tuple[bool, List[str]]:
        ...
        if max_edge > MAX_REASONABLE_EDGE:
            warnings.append(f"Edge anomaly: Max edge {max_edge:.1f} exceeds threshold {MAX_REASONABLE_EDGE}")
            has_anomaly = True

        if avg_edge > MAX_AVG_EDGE_PER_DAY:
            warnings.append(f"Edge anomaly: Avg edge {avg_edge:.1f} exceeds threshold {MAX_AVG_EDGE_PER_DAY}")
            has_anomaly = True
    ...
    def print_performance_report(self, performance: Dict, status: str, reasons: List[str]):
        ...
        print(f"  - Wins: {overall['wins']} | Losses: {overall['losses']} | Pending: {overall['pending']}")
        ...
        for market, metrics in performance['by_market'].items():
            ...
            print(f"\n  {status_icon} {market}:")
            print(f"     - Picks: {metrics['total_picks']} | Wins: {metrics['wins']} | Losses: {metrics['losses']}")
            print(f"     - Win Rate: {metrics['win_rate']:.1f}% (Benchmark: {benchmark_wr:.1f}%, Diff: {wr_diff:+.1f}pp)")
