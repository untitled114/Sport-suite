...
class PerformanceCalculator:
    ...
    def load_picks(self):
        ...
        self.picks_df = pd.read_sql_query(query, self.conn, params=(self.days,))
    ...
    def calculate_rolling_metrics(self):
        ...
        if len(recent_7d) > 0:
            ...
            roi_7d = (
                recent_7d["profit"].sum() / (wins_7d + losses_7d) * 100
                if (wins_7d + losses_7d) > 0
                else 0
            )
        else:
        ...
        if len(recent_30d) > 0:
            ...
            win_rate_30d = (
                wins_30d / (wins_30d + losses_30d) * 100 if (wins_30d + losses_30d) > 0 else 0
            )
            roi_30d = (
                recent_30d["profit"].sum() / (wins_30d + losses_30d) * 100
                if (wins_30d + losses_30d) > 0
                else 0
            )
        else:
    ...
    def generate_report(self, output_file=None):
        ...
        for market, stats in sorted(by_market.items()):
            ...
            if market in VALIDATION_BENCHMARKS:
                ...
                report_lines.append(
                    f"- Win Rate: {val['win_rate']:.1f}% → {stats['win_rate']:.1f}% ({wr_diff:+.1f}%)"
                )
                report_lines.append(
                    f"- ROI: {val['roi']:+.2f}% → {stats['roi']:+.2f}% ({roi_diff:+.2f}%)\n"
                )
        ...
        if high_spread:
            ...
            report_lines.append(f"**Validation Comparison**:")
            report_lines.append(
                f"- Win Rate: {val['win_rate']:.1f}% → {high_spread['win_rate']:.1f}% ({wr_diff:+.1f}%)"
            )
            report_lines.append(
                f"- ROI: {val['roi']:+.2f}% → {high_spread['roi']:+.2f}% ({roi_diff:+.2f}%)\n"
            )
...
def main():
    parser = argparse.ArgumentParser(description="Calculate NBA XL system performance metrics")
    parser.add_argument(
        "--days", type=int, default=30, help="Number of days to analyze (default: 30)"
    )
    parser.add_argument("--output", help="Output file path for report (markdown)", default=None)
