...
class CheatsheetPicksGenerator:
        ...
        for prop in props:
            ...
            if min_line is not None:
                ...
                if prop_line < min_line:
                    logger.debug(f"Filtered {prop.get('player_name')} {stat_type} - line {prop_line} < {min_line}")
                    continue
            ...
            if filter_min_line is not None:
                ...
                if prop_line < filter_min_line:
                    logger.debug(f"Filtered {prop.get('player_name')} {stat_type} - line {prop_line} < {filter_min_line} (filter-specific)")
                    continue
            ...
            if days_out is not None and days_out > MAX_DAYS_SINCE_LAST_GAME:
                logger.info(f"[SKIP] {prop.get('player_name')} - returning from {days_out} days out")
                continue
    ...
    def generate_picks(self) -> List[Dict]:
        ...
        for filter_name, filter_config in PRO_FILTERS.items():
            ...
            if filtered:
                logger.info(f"[OK] {filter_name}: {len(filtered)} picks (expected {filter_config['expected_wr']:.1f}% WR)")
    ...
    def _print_summary(self, output: Dict) -> None:
        ...
        if output['picks']:
            ...
            for i, pick in enumerate(output['picks'][:5], 1):
                ...
                print(f"   L5 Hit Rate: {pick['hit_rate_l5']*100:.0f}% | Opp Rank: #{pick['opp_rank']}")
...
def main():
    parser = argparse.ArgumentParser(
        description='Generate picks from BettingPros cheat sheet'
    )
    parser.add_argument(
        '--date',
    ...
    # Run generator
    generator = CheatsheetPicksGenerator(
        game_date=args.date,
