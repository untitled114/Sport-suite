...
def load_to_database(props: list, batch_size: int = 1000):
    ...
    values = [
        (
            p['player_name'], p['game_date'], p['stat_type'], p['platform'], p['line'],
            p['over_odds'], p['under_odds'], p['projection'], p['projection_diff'], p['bet_rating'],
            p['expected_value'], p['ev_pct'], p['probability'], p['recommended_side'],
            p['opp_rank'], p['opp_value'], p['hit_rate_l5'], p['hit_rate_l15'], p['hit_rate_season'],
            p['l5_over'], p['l5_under'], p['l15_over'], p['l15_under'], p['season_over'], p['season_under'],
            p['fetch_timestamp'], p['use_for_betting']
        )
        for p in transformed
    ]
