...
# Add parent for betting_xl imports
parent_dir = os.path.dirname(os.path.dirname(script_dir))
if parent_dir not in sys.path:
    sys.path.insert(0, parent_dir)

# Import opponent mapper for enriching props with opponent_team and is_home
...
class PropsLoader:
    ...
    def __init__(self, verbose: bool = True, use_mongodb: bool = True):
        ...
        self.mongo_buffer = defaultdict(lambda: {
            'player_name': None,
            'game_date': None,
            'stat_type': None,
            'opponent_team': None,
    ...
    def connect(self):
        ...
        if self.verbose:
            print(f"Connecting to PostgreSQL: {DB_CONFIG['database']}@{DB_CONFIG['host']}:{DB_CONFIG['port']}")
        ...
        if self.use_mongodb:
            try:
                if self.verbose:
                    print(f"Connecting to MongoDB: {MONGO_CONFIG['database']}.{MONGO_CONFIG['collection']}")
    ...
    def _buffer_mongodb_prop(self, prop: Dict[str, Any]):
        ...
        if self.mongo_buffer[key]['player_name'] is None:
            self.mongo_buffer[key].update({
                ...
                'game_date': datetime.fromisoformat(game_date) if isinstance(game_date, str) else game_date,
                'stat_type': stat_type,
                'opponent_team': prop.get('opponent_team'),
                'is_home': prop.get('is_home'),
                'game_id': prop.get('game_id', ''),
                'game_time': prop.get('game_time'),
                'actual_value': float(prop['actual_value']) if prop.get('actual_value') is not None else None
            })
        ...
        book_doc = {
            ...
            'under_odds': prop.get('under_odds', -110),
            'fetch_timestamp': datetime.fromisoformat(prop.get('fetch_timestamp')) if isinstance(prop.get('fetch_timestamp'), str) else datetime.now()
        }
    ...
    def _flush_mongodb_buffer(self):
        ...
        for key, doc in self.mongo_buffer.items():
            try:
                ...
                line_std_dev = (sum((x - consensus_line) ** 2 for x in lines) / len(lines)) ** 0.5 if len(lines) > 1 else 0.0
                line_coef_variation = (line_std_dev / consensus_line * 100) if consensus_line > 0 else 0.0
    ...
    def get_line_shopping_summary(self, game_date: str = None):
        ...
        if self.verbose:
            ...
            for row in results:
                ...
                print(f"  Avg spread:  {avg_spread:.2f} points" if avg_spread else "  Avg spread:  N/A")
                print(f"  Max spread:  {max_spread:.2f} points" if max_spread else "  Max spread:  N/A")
                print(f"  Spread ≥1.5: {opp_1_5} props ({opp_1_5/num_props*100:.1f}%)" if num_props > 0 else "")
                print(f"  Spread ≥2.5: {opp_2_5} props ({opp_2_5/num_props*100:.1f}%)" if num_props > 0 else "")
                print(f"  Spread ≥3.5: {opp_3_5} props ({opp_3_5/num_props*100:.1f}%)" if num_props > 0 else "")
...
def main():
    ...
    try:
        ...
        if combo_skipped:
            ...
            combo_types = ', '.join(f"{k.replace('combo_', '')}:{v}" for k, v in sorted(combo_skipped.items()))
