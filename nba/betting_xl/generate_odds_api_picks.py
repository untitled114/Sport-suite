...
class OddsApiPicksGenerator:
    ...
    def load_cheatsheet_data(self):
        ...
        logger.info(f"[DATA] Found {len(self.cheatsheet_data)} cheatsheet props for {self.game_date}")
        ...
        for filter_name, filter_config in ODDS_API_FILTERS.items():
            ...
            for prop in merged_props:
                ...
                if days_out is not None and days_out > MAX_DAYS_SINCE_LAST_GAME:
                    logger.info(f"[SKIP] {prop['player_name']} - returning from {days_out} days out")
                    continue
    ...
    def _build_pick(self, prop: Dict, filter_name: str, filter_config: Dict) -> Dict:
        ...
        return {
            ...
            'confidence': 'ELITE' if expected_wr >= 90 else ('HIGH' if expected_wr >= 75 else 'MEDIUM'),
    ...
    def save_picks(self, output_file: str, dry_run: bool = False) -> None:
        ...
        output = {
            ...
            'summary': {
                ...
                'by_stat_type': {},
                'by_filter': {},
                'high_confidence': len([p for p in self.picks if p['confidence'] in ('ELITE', 'HIGH')]),
            },
            'data_sources': {
    ...
    def _print_summary(self, output: Dict) -> None:
        ...
        if output['picks']:
            ...
            for i, pick in enumerate(output['picks'], 1):
                ...
                print(f"   Projection: {pick.get('projection', 0):.1f} (+{pick.get('projection_diff', 0):.1f})")
...
def main():
    ...
    parser.add_argument(
        '--pick6-file',
        default=None,
        help='Historical Pick6 JSON file (for backtest mode)'
    )
    parser.add_argument(
        '--dry-run',
        action='store_true',
