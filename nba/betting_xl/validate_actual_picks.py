...
def validate_pick(pick: dict, actual_value: float) -> dict:
    ...
    return {
        ...
        'line': best_line,
        'actual': actual_value,
        'edge': pick.get('top_3_lines', [{}])[0].get('edge_pct', 0) if pick.get('top_3_lines') else 0
    }
...
def print_results(all_results: list):
    ...
    for result in all_results:
        ...
        if picks_with_data > 0:
            ...
            print(f"{result['date']:<12} {result['total']:<6} {result['wins']:<4} {result['losses']:<4} {result['pushes']:<4} {wr:<7.1f} {roi_day:+<8.2f}")
    ...
    for market, stats in sorted(by_market.items()):
        ...
        if total_market > 0:
            ...
            print(f"{market:<12} {stats['wins']:<4} {stats['losses']:<4} {stats['pushes']:<4} {wr:<7.1f} {roi_market:+<8.2f}")
    ...
    for tier, stats in sorted(by_tier.items()):
        ...
        if total_tier > 0:
            ...
            print(f"{tier:<12} {stats['wins']:<4} {stats['losses']:<4} {stats['pushes']:<4} {wr:<7.1f} {roi_tier:+<8.2f}")
