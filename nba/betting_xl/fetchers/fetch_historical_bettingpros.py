...
class HistoricalBettingProsFetcher(BaseFetcher):
    ...
    def _parse_historical_response(
        self, data: Dict[str, Any], player_name: str, market_name: str, book_name: str, season: int
    ) -> List[Dict[str, Any]]:
        """
    ...
    def fetch_all_players(
        ...
        print(
            f"Expected API calls: {len(player_names) * (len(markets) if markets else 4) * (len(books) if books else 7)}"
        )
        print(
            f"Estimated time: {len(player_names) * (len(markets) if markets else 4) * (len(books) if books else 7) / 60:.1f} minutes"
        )
        print(f"{'='*70}\n")
        ...
        for i, player_name in enumerate(player_names, 1):
            ...
            player_props = self.fetch_player_all_markets_all_books(
                player_name=player_name, season=season, markets=markets, books=books
            )
            ...
            if i % save_frequency == 0:
                checkpoint_file = self.save_to_json(
                    props=all_props, suffix=f"season_{season}_checkpoint_{i}players"
                )
...
def main():
    ...
    parser.add_argument(
        "--players-file", type=str, help="Text file with player names (one per line)"
    )
    ...
    parser.add_argument(
        "--save-frequency", type=int, default=10, help="Save checkpoint every N players"
    )
    ...
    try:
        ...
        # Save final output
        output_file = fetcher.save_to_json(props=all_props, suffix=f"season_{args.season}_COMPLETE")
