...
class VegasLinesFetcher:
    ...
    def _parse_spread_or_ml(self, offer: Dict, selections: List, market_type: str) -> Dict:
        ...
        if len(participants) >= 2:
            ...
            result['home_team'] = participants[0].get('id') or participants[0].get('team', {}).get('abbreviation')
            result['away_team'] = participants[1].get('id') or participants[1].get('team', {}).get('abbreviation')
    ...
    def fetch_all_lines(self) -> List[Dict]:
        ...
        for event in self.events:
            ...
            # Log summary
            spread_str = f"{game_lines['spread_consensus']}" if game_lines['spread_consensus'] else "N/A"
            total_str = f"{game_lines['total_consensus']}" if game_lines['total_consensus'] else "N/A"
        ...
        for game in lines:
            ...
            try:
                ...
                if cursor.rowcount > 0:
                    ...
                    inserted += 1
                    logger.info(f"Inserted {game['away_team']} @ {game['home_team']}: "
                               f"spread={game['spread_consensus']}, total={game['total_consensus']}")
                    conn.commit()
