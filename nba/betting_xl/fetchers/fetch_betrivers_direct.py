#!/usr/bin/env python3
"""
BetRivers Direct Fetcher (Kambi Backend)
==========================================
Fetches NBA player props directly from the Kambi sportsbook API that powers BetRivers.

API Flow:
1. GET /listView/basketball/nba.json  → list of NBA events with event IDs
2. GET /betoffer/event/{event_id}.json → all bet offers for that event (player props)
...
Usage:
    python fetch_betrivers_direct.py                # Fetch today's props
    python fetch_betrivers_direct.py --save         # Save to JSON
    python fetch_betrivers_direct.py --quiet        # Quiet mode
"""
...
# Kambi API endpoints for Rush Street Interactive (Colorado jurisdiction)
# Multiple domains/subdomains — Kambi Varnish rate-limits per IP so we try fallbacks
KAMBI_ENDPOINTS = [
    "https://eu-offering-api.kambicdn.com/offering/v2018/rsico",
    "https://eu-offering.kambicdn.org/offering/v2018/rsico",
    "https://us1-api.aws.kambicdn.com/offering/v2018/rsico",
]
KAMBI_BASE = KAMBI_ENDPOINTS[0]

# Kambi criterion labels that map to our stat types.
# The label format is "Player {Stat} - Over/Under" or similar.
STAT_LABEL_MAP = {
    "points": "POINTS",
    "rebounds": "REBOUNDS",
    "assists": "ASSISTS",
...
# Core stats the XL system cares about
...
def _decimal_to_american(decimal_odds: float) -> int:
    ...
    Args:
        decimal_odds: Decimal odds (e.g. 1.91, 2.10).
...
def _extract_stat_type(label: str) -> Optional[str]:
    ...
    Examples:
        ...
        "Player Three Point Field Goals - Over/Under" → "THREES"
    """
    label_lower = label.lower()
    # Must contain "player" to be a player prop
...
def _normalize_team(name: str) -> Optional[str]:
    ...
    Kambi uses full city+mascot names like "Los Angeles Lakers".
    Falls back to the team_utils utility for robust matching.
    """
    if not name:
        return None
    try:
...
class BetRiversDirectFetcher(BaseFetcher):
    """Fetches NBA player props from the Kambi API powering BetRivers."""

    def __init__(self, verbose: bool = True):
        super().__init__(
            source_name="betrivers_direct",
            rate_limit=5.0,  # Kambi Varnish is very aggressive — 5s minimum
            max_retries=3,
            timeout=30,
            verbose=verbose,
            proxy_profile="sportsbooks",  # Use proxy — server IP is burned by Kambi Varnish
        )
        self.today_est = datetime.now(EST).strftime("%Y-%m-%d")
        self._active_base = KAMBI_BASE  # May switch to fallback on 429/410
        ...
        for attempt in range(self.max_retries):
            ...
            try:
                ...
                if response.status_code == 410:
                    # 410 Gone = permanent IP ban by Kambi Varnish
                    ...
                    for fallback in KAMBI_ENDPOINTS:
                        ...
                        return None  # All endpoints banned
                    continue
                ...
                if response.status_code == 404:
                    # Event may have been removed (game started, etc.)
    ...
    def _fetch_events(self) -> List[Dict]:
        ...
        Returns list of event dicts, each containing .event metadata
        and optionally .betOffers for basic markets.
        """
        ...
        logger.info("Fetching Kambi NBA event list...")
    ...
    # ------------------------------------------------------------------
    # Step 2: Fetch all bet offers for one event
    # ------------------------------------------------------------------
    ...
    def _fetch_event_offers(self, event_id: int) -> List[Dict]:
        ...
        Args:
            event_id: Kambi event ID.

        Returns:
            List of betOffer dicts.
        """
    ...
    def _parse_event_metadata(
        ...
        """Extract event metadata (id, home, away, date, time) from a Kambi event wrapper.

        Kambi event list entries have structure:
            { "event": { "id": ..., "start": "...", "path": [...] },
              "betOffers": [...] }

        Participants are in event_wrapper["event"]["path"] or a top-level list.
        """
        ...
        # Parse teams from participants (homeName / awayName or path)
        home_team = None
        away_team = None
        ...
        # Fallback: try event name "Away @ Home" or "Away vs Home"
    ...
    def _parse_bet_offers(
        ...
        for offer in bet_offers:
            ...
            # Parse outcomes — group Over/Under pairs by player
            ...
            for outcome in outcomes:
                ...
                if side_label not in ("over", "under"):
                    # Sometimes the label is the player name and type encodes side
                ...
                # Player name — Kambi puts it in "participant" or "label" when
                # the market is per-player. For grouped markets the participant
                # field is populated.
                player_name = outcome.get("participant", "")
                if not player_name:
                    # Try the criterion label for single-player markets
                    # e.g. "LeBron James - Player Points"
    ...
    def fetch(self) -> List[Dict[str, Any]]:
        """Fetch all NBA player props from BetRivers (Kambi backend).

        Flow:
        1. Get NBA event list (single request).
        2. For each today's event, fetch all bet offers (one request per event).
        3. Parse player prop Over/Under markets from bet offers.
        """
        print("\n" + "=" * 70)
        print("FETCHING BETRIVERS NBA PROPS (Kambi Backend)")
        print("=" * 70)
        ...
        # Filter to today's events and extract metadata
        ...
        for event_id, home_team, away_team, game_date, game_time in today_events:
            ...
            # Respect rate limit — Kambi Varnish needs generous delays
            time.sleep(random.uniform(5.0, 8.0))

            offers = self._fetch_event_offers(event_id)
            if not offers:
                print(f"  [--] No bet offers returned", flush=True)
                continue

            # Parse player props from offers
        ...
        # Summary
        print("\n" + "=" * 70)
        print("BETRIVERS (KAMBI) FETCH SUMMARY")
        print("=" * 70)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Fetch NBA player props from BetRivers (Kambi backend)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Fetch today's props
  python fetch_betrivers_direct.py

  # Save to JSON file
  python fetch_betrivers_direct.py --save

  # Quiet mode
  python fetch_betrivers_direct.py --quiet
        """,
    )
    ...
    with BetRiversDirectFetcher(verbose=not args.quiet) as fetcher:
        props = fetcher.fetch()
