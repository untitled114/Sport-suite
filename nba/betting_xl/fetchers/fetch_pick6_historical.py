...
def get_historical_events(date_str: str) -> List[Dict]:
    ...
    logger.info(f"  [{date_str}] {len(events)} events (snapshot: {timestamp}, cost: {cost}, remaining: {remaining})")
    return events
...
def parse_event_props(event_data: Dict, game_date: str) -> List[Dict]:
    ...
    for bookmaker in event_data.get('bookmakers', []):
        ...
        main_keys = {(p['player'].lower(), p['stat_type']) for p in props if p['event_id'] == event_id}
...
def main():
    ...
    parser.add_argument('--output', default='the-odds-api-data/historical_pick6_backfill.json',
    ...
    if args.dry_run:
        ...
        est_cost = total_events * len(MARKETS_TO_FETCH) * 10 + (end - datetime.strptime(args.start, '%Y-%m-%d')).days + 1
        logger.info(f"\nDry run results:")
