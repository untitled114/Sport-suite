...
class StackedMarketModel:
    ...
    def load_data(self, data_path: str) -> pd.DataFrame:
        ...
        if before_dedup != after_dedup:
            logger.info(f"   ✅ Dropped {before_dedup - after_dedup:,} duplicates → {after_dedup:,} unique props")
    ...
    def prepare_features(self, df: pd.DataFrame):
        ...
        exclude_cols = [
            'player_id', 'player_name', 'game_date', 'date', 'player', 'team_abbrev', 'opponent_abbrev',
            'actual_points', 'actual_rebounds', 'actual_assists', 'actual_threes',
            'actual_result', 'season', 'opponent', 'stat_type', 'source', 'prop_id',
            ...
            'split', 'label',  # Train/val split indicator and binary label (targets, must exclude!)
            'hit_over', 'residual'  # CRITICAL: Binary target + residual - exclude to prevent data leakage
        ]
        ...
        if len(book_features) == 0:
            logger.warning("   ⚠️  No book encoding features found - training without market-aware features")
        ...
        logger.info(f"   Target (residual) stats: mean={y_residual.mean():.2f}, std={y_residual.std():.2f}")
    ...
    def train(self, X_train, y_value_train, y_binary_train, y_residual_train,
              X_test, y_value_test, y_binary_test, y_residual_test):
        """
        ...
        logger.info(f"   Expected diff train: mean={expected_diff_train.mean():.2f}, std={expected_diff_train.std():.2f}")
        logger.info(f"   Expected diff test: mean={expected_diff_test.mean():.2f}, std={expected_diff_test.std():.2f}")
        ...
        logger.info(f"   Class distribution: UNDER={sum(y_binary_train==0)}, OVER={sum(y_binary_train==1)}")
        ...
        print(classification_report(y_binary_test, y_pred_binary_test, target_names=['UNDER', 'OVER']))
        ...
        y_binary_cal_train = y_binary_train.iloc[cal_train_idx].values if hasattr(y_binary_train, 'iloc') else y_binary_train[cal_train_idx]
        y_binary_cal_val = y_binary_train.iloc[cal_val_idx].values if hasattr(y_binary_train, 'iloc') else y_binary_train[cal_val_idx]
        ...
        logger.info(f"   Calibrator fitted on {len(y_prob_cal_val)} samples from training data (no test data leakage)")
        ...
        y_prob_blend_test = (
            blend_weight_cls * y_prob_test_cal +
            blend_weight_res * prob_from_residual_test
        )
        y_prob_blend_train = (
            blend_weight_cls * y_prob_train_cal +
            blend_weight_res * prob_from_residual_train
        )
...
def main():
    ...
    try:
        ...
        X_train, X_test, y_value_train, y_value_test, y_binary_train, y_binary_test, y_residual_train, y_residual_test = train_test_split(
            X, y_value, y_binary, y_residual,
        ...
        # Train
        metrics = model.train(
            X_train, y_value_train, y_binary_train, y_residual_train,
            X_test, y_value_test, y_binary_test, y_residual_test
        )
