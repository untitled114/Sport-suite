...
DB_CONFIG = {
    'host': 'localhost', 'port': 5536, 'database': 'nba_players',
    'user': os.getenv('DB_USER', 'nba_user'), 'password': os.getenv('DB_PASSWORD')
}
...
def load_to_db(df, conn):
    ...
    for _, row in df.iterrows():
        ...
        insert_data.append((
            int(row['PLAYER_ID']), str(row.get('GAME_ID', '')), game_date, season_year,
            ...
            int(row.get('STL', 0)), int(row.get('BLK', 0)), int(row.get('TOV', 0)),
            int(row.get('FG3M', 0)), int(row.get('FGM', 0)), int(row.get('FGA', 0)),
            int(row.get('FG3A', 0)), int(row.get('FTM', 0)), int(row.get('FTA', 0)),
