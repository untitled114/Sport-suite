...
def load_team_pace() -> Dict[str, float]:
    ...
    if latest_season != CURRENT_SEASON:
        logger.info(
            f"Using team pace from season {latest_season} (current season pace not available)"
        )
            ...
            if len(games_so_far) >= 5:
                ...
                if season_avg_points > 0:
                    is_hot = (l5_avg_points > season_avg_points * 1.2) or (
                        season_std_points > 0
                        and l5_avg_points > season_avg_points + season_std_points
                    )
    ...
    try:
        ...
        for idx, row in players_to_update.iterrows():
            records = update_player_rolling_stats(
                conn, row["player_id"], row["player_name"], last_update, team_pace_map
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Incrementally update NBA rolling stats")
    parser.add_argument(
        "--force-date", help="Force update from this date (YYYY-MM-DD)", default=None
    )
