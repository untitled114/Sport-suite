        ...
        for team in data.get('injuries', []):
            ...
            for inj in team.get('injuries', []):
                ...
                injuries.append({
                    ...
                    'espn_id': athlete.get('links', [{}])[0].get('href', '').split('/')[-1] if athlete.get('links') else None,
...
def load_to_database(injuries):
    ...
    try:
        ...
        player_cursor.execute("SELECT DISTINCT ON (full_name) player_id, full_name FROM player_profile ORDER BY full_name, player_id DESC")
        player_map = {full_name.lower(): player_id for player_id, full_name in player_cursor.fetchall()}
        ...
        for inj in injuries:
            try:
                ...
                status = inj['status'].upper().replace(' ', '_').replace('-', '_')[:50]  # Truncate status too
